  ...
  export default {
    ...
    methods: {
      ...
      onSearch (page = 1) {
        ...
        this.loading = true
      // 发送请求
        this.$axios.get(SITEURL.RECHARGE_SEARCH + '?first_time=' + this.formInline.fromDate + '&area=' + text + '&area_type=' + num + '&page=' + page + '&limit=15' + '&last_time=' + this.formInline.toDate).then(response => {
          this.loading = false
          this.show = true
